...
import me.emmy.pluginbase.locale.ConfigLocale;
...
public class CommandFramework implements CommandExecutor {
    ...
    private void defaultCommand(CommandArgs args) {
        ...
        if (args.getSender().hasPermission(PluginBase.getInstance().getConfig("settings.yml").getString("command.syntax-bypass-perm"))) {
            if (parts.length > 1) {
            ...
            args.getSender().sendMessage(CC.translate(PluginBase.getInstance().getConfig("settings.yml").getString("command.anti-syntax-message").replace("{argument}", args.getLabel())));
        }
    }
}